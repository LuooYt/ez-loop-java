...
public class TaskCreateTool implements Tool {
    ...
    /**
     * 返回输入 JSON Schema，定义 description（必填）与 metadata（可选）参数。
     */
    @Override
        ...
        return PromptI18n.t(PromptI18n.toolSchemaKey(name()), """
                {
                  ...
                  "properties": {
                    "description": {
                      "type": "string",
                      "description": "任务描述，说明该任务要实现的目标"
                    },
                    "metadata": {
                      "type": "string",
                      "description": "可选的 JSON 元数据字符串，例如 {\\"priority\\":\\"high\\"}"
                    }
                  },
    ...
    /**
     * 创建手动管理模式的后台任务：校验 description 参数、解析可选 metadata，
     * 委托 TaskManager 创建任务并返回包含 task_id 与 status 的 JSON 结果。
     */
    @Override

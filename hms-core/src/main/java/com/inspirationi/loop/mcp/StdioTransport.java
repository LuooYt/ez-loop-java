...
public class StdioTransport implements McpTransport {
     ...
     * 发送 JSON-RPC 请求并等待响应。
     * <p>
     * 将请求写入子进程 stdin（一行一条），通过 {@link CompletableFuture} 等待
     * 读取线程从 stdout 返回的对应响应；若响应包含 {@code error} 字段则抛出异常。
     *
     * @param jsonRpcRequest 完整的 JSON-RPC 2.0 请求字符串（必须包含 id 字段）
     * @return 服务器返回的 JSON-RPC 响应节点
     * @throws McpException 未连接、缺少 id、超时或服务器返回错误
     */
    @Override
     ...
     * 发送 JSON-RPC 通知（无需响应）。
     * <p>
     * 将通知写入子进程 stdin（一行一条），不等待服务器回复。
     *
     * @param jsonRpcNotification 完整的 JSON-RPC 2.0 通知字符串
     * @throws McpException 未连接或写入失败
     */
    @Override
     ...
     * 传输层是否已连接且可用。
     *
     * @return 连接状态为真、进程存在且仍存活时返回 {@code true}
     */
    @Override
    ...
    /**
     * 关闭传输层：关闭 stdin、等待并终止子进程、中断读取线程并清理待处理请求。
     */
    @Override
